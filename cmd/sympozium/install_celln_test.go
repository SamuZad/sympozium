...
func TestSplitImageRef(t *testing.T) {
	cases := []struct {
		...
		{"", "ghcr.io/sympozium-ai/celln", "v0.5.27", false},
		{"ghcr.io/sympozium-ai/celln:v0.5.27", "ghcr.io/sympozium-ai/celln", "v0.5.27", false},
		{"ghcr.io/sympozium-ai/celln@sha256:abcdef", "ghcr.io/sympozium-ai/celln", "sha256:abcdef", true},
		{"localhost:5000/celln", "localhost:5000/celln", "v0.5.27", false},
		{"localhost:5000/celln:v1", "localhost:5000/celln", "v1", false},
	}
	for _, c := range cases {
		repo, ref, digest := splitImageRef(c.in, "ghcr.io/sympozium-ai/celln", "v0.5.27")
...
func TestCellnInstallSetValues(t *testing.T) {
	vals, err := cellnInstallSetValues(
		context.Background(),
		"ghcr.io/sympozium-ai/celln:v0.5.27",
		"",
	...
	if router["image"].(map[string]interface{})["tag"] != "v0.5.27" {
		t.Fatalf("router.image.tag = %v, want v0.5.27", router["image"])
	}
