...
func readBackendConfiguration(dir, backend, packageHash, principal string) (backendConfiguration, error) {
	...
	if c.APIVersion != "celln.native-starter-configured/v1" || c.ExecutionAuthorized || c.Readiness != "not_established" || b.native.ModelProfile != c.ModelProfile || c.Principal != principal || len(b.cat.Tools) != 3 || b.cat.Worker.ContractVersion != "celln.json-tools/v1" {
